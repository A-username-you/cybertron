...
class SubdomainEnumPlugin(PluginInterface):
    name = "subdomain_enum"; version = "2.0.0"
    ...
    async def execute(self, target: str, options: Dict) -> TaskResult:
        ...
        findings = []; subdomains: Set[str] = set()
        try:
                ...
                try:
                    ...
                    if resolver:
                        ...
                        findings.append(Finding(
                            ...
                            target=f"{sub}.{target}", evidence={"ips": ips}))
                except Exception: pass
        await asyncio.gather(*[check_sub(w) for w in words[:1000]])
        return TaskResult(task_id=__import__("uuid").uuid4().hex,
                          status=TaskStatus.SUCCESS, findings=list(findings),
                          artifacts={"subdomains": list(subdomains)})

class PortScanPlugin(PluginInterface):
    name = "port_scan"; version = "2.0.0"
            ...
            async with semaphore:
                try:
                    ...
                    findings.append(Finding(
                        ...
                        severity=Severity.INFO, category="port_scan",
                        target=target, evidence={"port": port}))
                except (asyncio.TimeoutError, ConnectionRefusedError, OSError): pass
        await asyncio.gather(*[scan_port(p) for p in common_ports])
        return TaskResult(task_id=__import__("uuid").uuid4().hex,
                          status=TaskStatus.SUCCESS, findings=findings,
