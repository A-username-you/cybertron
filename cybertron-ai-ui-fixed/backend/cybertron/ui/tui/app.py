...
class CybertronTUI(App):
    CSS="""Screen{align:center middle;} DataTable{height:1fr;} Log{height:1fr;} #sidebar{width:30%;} #main{width:70%;} .title{text-align:center;text-style:bold;color:$primary;}"""
    BINDINGS=[("q","quit","Quit")]
    findings=reactive([])
    def __init__(self,engine:ExecutionEngine): super().__init__(); self.engine=engine
    def compose(self)->ComposeResult:
        ...
        with Horizontal():
            ...
            with Vertical(id="main"):
                with TabbedContent():
                    with TabPane("Findings",id="findings"): yield DataTable(id="findings_table")
                    with TabPane("Log",id="log"): yield Log(id="log_widget")
        yield Footer()
    def on_mount(self)->None:
        t=self.query_one("#findings_table",DataTable); t.add_columns("Severity","Category","Target","Title")
    ...
    async def on_button_pressed(self,event:Button.Pressed)->None:
        ...
        log=self.query_one("#log_widget",Log); table=self.query_one("#findings_table",DataTable)
        if not target: log.write_line("[ERROR] No target"); return
