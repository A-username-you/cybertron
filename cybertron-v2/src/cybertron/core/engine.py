...
class ScopeManager:
    ...
    def is_in_scope(self, target: str) -> bool:
        if target in self.excluded_targets:
            return False
        if target in self.allowed_domains or target in self.allowed_ips:
            return True
    ...
    async def execute_pipeline(self, pipeline: List[Dict[str, Any]], target: str) -> List[TaskResult]:
        ...
        for step in pipeline:
            ...
            if condition and results:
                last = results[-1]
                if condition == "has_findings" and not last.findings:
                    continue
                if condition == "no_findings" and last.findings:
                    continue

            result = await self.execute_task(plugin_name, target, options, config)
            results.append(result)

            if step.get("stop_on_failure") and result.status == TaskStatus.FAILED:
                break
