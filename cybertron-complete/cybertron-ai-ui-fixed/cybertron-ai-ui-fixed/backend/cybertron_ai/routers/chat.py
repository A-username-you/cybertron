...
class ChatMessage(BaseModel):
    message: str; session_id: str = "default"; context: Dict[str,Any] = {}
class ChatResponse(BaseModel):
    response: str; intent: Dict[str,Any]; findings: list; engagement_id: str = None
...
async def chat_message(msg: ChatMessage, request: Request):
    ...
    if intent.intent == "scan":
        ...
        for r in results: findings.extend([f.model_dump() for f in r.findings])
        analysis = await ai_orch.analyze_findings([f for r in results for f in r.findings])
        response_parts.append(f"\n📊 **Risk Score:** {analysis.get('risk_score',0)}/100")
        response_parts.append(f"📝 **Summary:** {analysis.get('summary','N/A')}")
        if analysis.get('recommendations'):
            response_parts.append("\n🔧 **Recommendations:**")
            for rec in analysis['recommendations'][:3]: response_parts.append(f"   • {rec}")
    elif intent.intent == "engage":
        engagement = await state.orchestrator.start_engagement(name=f"AI-{msg.session_id[:8]}", target=intent.target, scope_file="configs/default_scope.yml")
        engagement_id = engagement.id
        response_parts.append(f"🎯 **Engagement started:** `{engagement.id}`")
...
async def chat_stream(msg: ChatMessage, request: Request):
    ...
    async def event_generator():
        ...
        if intent.intent in ("scan","audit"):
            ...
            yield f"data: {json.dumps({'type':'progress','content':f'Discovered {total} findings'})}\n\n"
            analysis = await state.ai_orchestrator.analyze_findings([f for r in results for f in r.findings])
