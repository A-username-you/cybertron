...
def send_slack(webhook_url: str, text: str, session_id: Optional[str] = None) -> bool:
    ...
    payload = {
        "text": text,
        "blocks": [
            {"type": "header", "text": {"type": "plain_text", "text": "Cybertron Alert", "emoji": True}},
            {"type": "section", "fields": [
...
def notify_session_complete(
    webhook_url: str, slack_url: str, session_id: str, goal: str,
    state: str, findings: list, elapsed_ms: int,
):
    summary = f"Session {session_id[:8]} completed: {state}\nGoal: {goal}\nFindings: {len(findings)}\nElapsed: {elapsed_ms}ms"
    if webhook_url:
        send_webhook(webhook_url, {
            "event": "session_complete", "session_id": session_id,
            "goal": goal, "state": state, "findings_count": len(findings), "elapsed_ms": elapsed_ms,
        })
    if slack_url:
