...
{"intent":"scan|audit|analyze|engage","target":"domain","plugins":["plugin_name"],"options":{},"reasoning":"..."}"""
    ...
    def _fallback_intent(self, message: str) -> AgentIntent:
        msg = message.lower()
        plugins = []
        if "subdomain" in msg or "domain" in msg:
            plugins.append("subdomain_enum")
        if "port" in msg:
            plugins.append("port_scan")
        if "aws" in msg or "cloud" in msg:
            plugins.append("aws_misconfig")
        if "api" in msg or "openapi" in msg:
            plugins.append("openapi_fuzz")
        if not plugins:
            plugins = ["subdomain_enum", "port_scan"]
        return AgentIntent(
            intent="scan",
            target=self._extract_target(msg) or "example.com",
            plugins=plugins,
            options={},
    ...
    async def execute_intent(self, intent: AgentIntent, engagement_id: Optional[str] = None) -> List[Dict[str, Any]]:
        self.logger.info("executing_intent", intent=intent.intent, target=intent.target, plugins=intent.plugins)
        # Placeholder: in full version this calls the execution engine
        return [{"plugin": p, "findings": []} for p in intent.plugins]

    async def analyze_findings(self, findings: List[Any]) -> Dict[str, Any]:
        if not findings:
            return {"summary": "No findings.", "risk_score": 0, "recommendations": []}
        prompt = f"""Analyze these security findings:
...
Respond as JSON: {{"summary":"...","risk_score":N,"recommendations":["..."]}}"""
        response = await self.llm.chat_completion([
            {"role": "system", "content": "Senior security analyst."},
            {"role": "user", "content": prompt}
        ], temperature=0.3)
        try:
            return json.loads(response)
        except:
            return {"summary": response[:500], "risk_score": 50, "recommendations": ["Review manually"]}
